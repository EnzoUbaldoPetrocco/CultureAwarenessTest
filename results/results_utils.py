...
def print_stats(errs_pu, stds_pu, lamb, CICs, stdsCIC, tau):
    print(f'LAMBDA INDEX = {lamb} AND TAU = {tau}')
    errors = []
    for j in range(3):
        #acc, _ = retrieve_mean_dev_std_accs_pu(accs[j][lamb])
        #print(f'Accuracy is {acc:.1f}+-{stds_pu[j][lamb]:.1f} on Culture {j}')
        print(f'Error is {errs_pu[lamb][j]*100:.1f}%+-{stds_pu[lamb][j]*100:.1f}% on Culture {j}')
...
def retrieve_statistics(p, model, ns, pu):
    ...
    for i in range(len(pu)):
        ...
        lambdas = []
        taus = [0.05, 0.1, 0.2] # 5%, 10% and 20% of the minimum error
        if len(errs_pu)>0:
