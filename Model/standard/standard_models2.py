...
class StandardModels(GeneralModelClass):
    ...
    def newModelSelection(self, TS, VS, aug, show_imgs=False, batches=[32], lrs=[1e-2, 1e-3, 1e-4], fine_lrs=[1e-5, 1e-6], epochs=30, fine_epochs=10, nDropouts=[0.4], g=0.1):
        best_loss = np.inf
        for b in batches:
            for lr in lrs:
                for fine_lr in fine_lrs:
                    for nDropout in nDropouts:
                        with tf.device("/gpu:0"):
                            ...
                            history = self.newDL(TS, VS, aug, show_imgs, b, lr, fine_lr, epochs, fine_epochs, nDropout, g=g)
        ...
        with tf.device("/gpu:0"):
            ...
            TS = TS + VS
            self.newDL(TS, None, aug, show_imgs, best_bs, best_lr, best_fine_lr, epochs, fine_epochs, best_nDropout, val=False, g=g)
