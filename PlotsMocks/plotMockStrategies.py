...
def get_base_data():
    ...
    c1_1 = np.random.multivariate_normal([-0.6, 0.6], cov, N_MAJ)
    # Culture 2 (Minority) - Right/Bottom
    c2_0 = np.random.multivariate_normal([2.3, 0.7], cov, N_MIN)
    c2_1 = np.random.multivariate_normal([1.0, -1.3], cov, N_MIN)
...
for i, ax in enumerate(axes):
    ...
    if i == 1: # RT applied to BOTH minority classes
        ...
        synth_0 = np.random.multivariate_normal([2.3, 0.7], [[0.1, 0.03], [0.03, 0.1]], 120)
        synth_1 = np.random.multivariate_normal([1.0, -1.3], [[0.1, 0.03], [0.03, 0.1]], 120)
        ...
        s_maj1 = np.random.multivariate_normal([-0.6, 0.6], cov_broad, 200)
        s_min0 = np.random.multivariate_normal([2.3, 0.7], cov_broad, 150)
        s_min1 = np.random.multivariate_normal([1.0, -1.3], cov_broad, 150)
